...
param_plot_data_brier <- param_summary_all |>
  group_by(parameter) |>
  summarise(
    mean_tunability = mean(tunability_brier, na.rm = TRUE),
    min_tunability  = min(tunability_brier, na.rm = TRUE),
    max_tunability  = max(tunability_brier, na.rm = TRUE)
  )


library(ggplot2)

ggplot(param_plot_data_brier, aes(x = reorder(parameter, mean_tunability), 
                                  y = mean_tunability)) +
  geom_bar(stat  = "identity", 
           width = 0.6,
           fill = "gray") +
  geom_errorbar(aes(ymin = min_tunability, 
                    ymax = max_tunability), 
                width = 0.2, 
                color = "black") +
